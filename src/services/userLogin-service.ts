...
async function userLogin(email: string, password: string) {
    ...
    const token = jwt.sign({userId: user?.id, name: user?.username, isAdmin: user?.isAdmin}, `${process.env.JWT_SECRET}`)
    
    // Utilizo a biblioteca bcrypt para fazer a verificação de senha
    const senhaHash = bcrypt.compare(password, user?.password)
